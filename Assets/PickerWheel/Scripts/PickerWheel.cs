...
namespace EasyUI.PickerWheelUI {
   ...
   public class PickerWheel : MonoBehaviour {
      ...
      private void DrawPiece (int index) {
         ...
         pieceTrns.GetChild (1).GetComponent <TextMeshProUGUI> ().color = piece.LabelColor;
         pieceTrns.GetChild (2).GetComponent <TextMeshProUGUI> ().text = piece.Amount.ToString () ;
         pieceTrns.GetChild (2).GetComponent <TextMeshProUGUI> ().color = piece.AmountColor;

            //Line
         Transform lineTrns = Instantiate (linePrefab, linesParent.position, Quaternion.identity, linesParent).transform ;
         lineTrns.gameObject.GetComponent<Image>().color = lineColor;
            ...
            .OnUpdate (() => {
               ...
               if (diff >= halfPieceAngle) {
                  if (isIndicatorOnTheLine) {
                        ...
                        AudioManager.Instance.PlaySfx(tickAudioClip, volume);
                  }
